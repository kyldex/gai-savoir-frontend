...
const ArticlePage: NextPage<Props> = ({ article, preview }) => {
  return (
    <Article
      article={article}
      preview={preview}
      formattedSubcategoryName={FORMATTED_SUBCATEGORY_NAME.NEWS}
    />
  );
};
...
export const getStaticPaths: GetStaticPaths = async () => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/ideas?filters[$and][0][subcategory][name][$eq]=${SUBCATEGORIES.NEWS}`
  );
