...
const Interviews: NextPage<Props> = ({ audiovisualProductions }) => {
  return (
    <div className={styles.container}>
      <Head>
        <title>{SUBCATEGORIES.INTERVIEWS}</title>
        <meta
          name="description"
          content={`${SUBCATEGORIES.INTERVIEWS} - Productions audiovisuelles du Gai Savoir`}
        />
      ...
      <h1 className={styles.title}>{SUBCATEGORIES.INTERVIEWS}</h1>
      ...
      <div className={styles.cardsContainer}>
        {audiovisualProductions.map((audiovisualProduction) => (
          <ItemCard
            ...
            categorySlugPart="production-audiovisuelle/interviews"
...
export const getStaticProps: GetStaticProps = async () => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/audiovisual-productions?filters[$and][0][subcategory][name][$eq]=${SUBCATEGORIES.INTERVIEWS}&populate[0]=card_image&sort=publishedAt%3Adesc`
  );
